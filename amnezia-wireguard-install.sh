...
H2=${H2}
H3=${H3}
H4=${H4}" >/etc/amnezia/amneziawg/params
...
PostDown = firewall-cmd --remove-port ${SERVER_PORT}/udp && firewall-cmd --remove-rich-rule='rule family=ipv4 source address=${FIREWALLD_IPV4_ADDRESS}/24 masquerade' && firewall-cmd --remove-rich-rule='rule family=ipv6 source address=${FIREWALLD_IPV6_ADDRESS}/24 masquerade'" >>"/etc/amnezia/amneziawg/${SERVER_AWG_NIC}.conf"
	else
...
function newClient() {
	...
	until [[ ${CLIENT_NAME} =~ ^[a-zA-Z0-9_-]+$ && ${CLIENT_EXISTS} == '0' && ${#CLIENT_NAME} -lt 16 ]]; do
		...
		CLIENT_EXISTS=$(grep -c -E "^### Client ${CLIENT_NAME}\$" "/etc/amnezia/amneziawg/${SERVER_AWG_NIC}.conf")
	...
	for DOT_IP in {2..254}; do
		DOT_EXISTS=$(grep -c "${SERVER_AWG_IPV4::-1}${DOT_IP}" "/etc/amnezia/amneziawg/${SERVER_AWG_NIC}.conf")
...
function listClients() {
	NUMBER_OF_CLIENTS=$(grep -c -E "^### Client" "/etc/amnezia/amneziawg/${SERVER_AWG_NIC}.conf")
	...
	grep -E "^### Client" "/etc/amnezia/amneziawg/${SERVER_AWG_NIC}.conf" | cut -d ' ' -f 3 | nl -s ') '
}
...
function revokeClient() {
	...
	CLIENT_NAME=$(grep -E "^### Client" "/etc/amnezia/amneziawg/${SERVER_AWG_NIC}.conf" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)

	# remove [Peer] block matching $CLIENT_NAME
	sed -i "/^### Client ${CLIENT_NAME}\$/,/^$/d" "/etc/amnezia/amneziawg/${SERVER_AWG_NIC}.conf"
